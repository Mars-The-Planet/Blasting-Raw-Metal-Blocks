...
public class CommonClass {
    public static void init() {
        ...
        for(String rawMetal : BlastingRawConfig.raw_metal_list){
            ...
            DeimosRecipeGenerator.createBlastingJson(rawMetalSet[0], rawMetalSet[1], BlastingRawConfig.cooking_time, Float.parseFloat(rawMetalSet[2]));
            if(BlastingRawConfig.works_in_furnace)
                DeimosRecipeGenerator.createSmeltingJson(rawMetalSet[0], rawMetalSet[1], BlastingRawConfig.cooking_time * 2, Float.parseFloat(rawMetalSet[2]));
        }
    }
}
